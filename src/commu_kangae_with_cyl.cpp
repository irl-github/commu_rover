...
int main(int argc, char **argv) {
  ...
  while(ros::ok()) {
	...
	if(difftime(time(0), afterIniTime) < forwardingDuration){
	  ...
	  if((difference >= (forwardingDuration + 1.5)) && (difference < (forwardingDuration + 1.65))){
	    msg.linear.z = 0.05;
	    count1++;
	    ROS_INFO("count1,count2,count3 = %d , %d , %d",count1,count2,count3);
	    ROS_INFO("diff : %lf",difference);
	  }
	  else if((difference >= (forwardingDuration + 1.65)) && (difference < (forwardingDuration + 1.8))){
