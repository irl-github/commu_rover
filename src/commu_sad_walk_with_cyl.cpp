...
int main(int argc, char **argv) {
  ...
  char const *position = "/gesture sad_position";
  char const *hello = "/gesture sad_walk"; //kyorokyoro";
  ...
  while(ros::ok()) {
	...
	//ROS_INFO("%d",(int)difference % 90);
	
	if(((int)difference % 90 >= 0) && ((int)difference % 90 <= 30) && (kyoroLoop == true)){
	  kyoroLoop = false;
	  send(sock , hello , strlen(hello) , 0 );
	  count++;
	  ROS_INFO("send %d",count);
	}else if((int)difference % 90 >= 30) kyoroLoop = true;
	  


	
	if(((int)difference % 90 >= 30) && ((int)difference % 90 < 60)){
	  //kyoroTime = time(0);
	  //ROS_INFO("1 diff : %lf",difference);  
	  msg.linear.z =0.03;
	}
	else if(((int)difference % 90 >= 60) && ((int)difference % 90 < 90)){
