...
int main(int argc, char **argv) {
     ...
     const float forwardingDuration = 7.0;
	 ...
	 if(timeOfFirstLoop){afterIniTime = time(0); timeOfFirstLoop = false;}
	 
	 if(difftime(time(0), afterIniTime) < forwardingDuration) msg.linear.x = linSpeed;
	 else msg.linear.x = 0;
       }
/*       
       if(difftime(time(0),firstTime) < forwardingDuration ) {
	 msg.linear.x = linSpeed;
	 ROS_INFO("YES: %f", difftime(time(0),firstTime));
       }
       else {
	 msg.linear.x = linSpeed;
	 ROS_INFO("NO: %f", difftime(time(0),firstTime));
       }
*/       
