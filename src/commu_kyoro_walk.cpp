...
int main(int argc, char **argv) {
  ...
  char const *walk = "/gesture walk"; //kyorokyoro";
  ...
  while(ros::ok()) {
	...
	after = getMicrotime();
      ...
      if(int(difference)%15>=0 && int(difference)%15<=5 && kyoroLoop == true){
	kyoroLoop = false;
	kyoroTime = time(0);
	...
	kyoroLoop = true;
	 ...
	 if(kyoroLoop == true){
      ...
      }else if((int)(difference)%150 >= 10){
	kyoroLoop = true;
	}*/

      /* if((int)(difference)%12 > 0 && (int)(difference)%12<=6) {
	msg.linear.z = -0.04;
	ROS_INFO("1 %d",(int)(difference)%10);
	}	*/  
	...
	ROS_INFO("3 %d",(int)(difference)%150);
      ...
      /*else{
	msg.linear.z = 0.04;
	ROS_INFO("4 %d",(int)(difference)%10);
      }*/
	  
      if(((int)difference % 10 >= 0) && ((int)difference % 10 <= 5) && (walkLoop == true)){
	walkLoop = false;
	send(sock , walk , strlen(walk) , 0 );
	ROS_INFO("walk");
      }

      else if((int)difference % 10 >= 5) walkLoop = true;

        if(((int)difference % 10 >= 0) && ((int)difference % 10 < 5)){
	  //kyoroTime = time(0);
	  ROS_INFO("1 diff : %lf",difference);  
	  msg.linear.z =0.04;
	}
	else if(((int)difference % 10 >= 5) && ((int)difference % 10 < 10)){
	  //if(difftime(time(0), kyoroTime) > 14){
	  kyoroLoop = true;
	  msg.linear.z =-0.04;
