...
int main(int argc, char **argv) {
  ...
  char const *hello = "/gesture ojigi"; //ojigi";
  char const *init = "/gesture baibai";
  ...
  while(ros::ok()) {
	...
	if(difftime(time(0), afterIniTime) < forwardingDuration){
	  ...
	  if(difftime(time(0), afterIniTime) >= 3.0 && difftime(time(0), afterIniTime) <= 4.0) msg.linear.z = -cylSpeed;
	  else if(difftime(time(0), afterIniTime) >= 4.0 && difftime(time(0), afterIniTime) <= 5.0) msg.linear.z = cylSpeed;
	  else msg.linear.z = 0;
	}
	else{
	  if(difftime(time(0), afterIniTime) > 6  && initFlag == true){
	    initFlag = false;
	    send(sock , init , strlen(init) , 0 );
	    ROS_INFO("baibai");
	  }
	  if(difftime(time(0), afterIniTime) >= 8.0 && difftime(time(0), afterIniTime) <= 9.0) msg.linear.z = -cylSpeed;
	  else if(difftime(time(0), afterIniTime) >= 9.0 && difftime(time(0), afterIniTime) <= 10.0) msg.linear.z = cylSpeed;
	  else msg.linear.z = 0;
	}
      }
    }
