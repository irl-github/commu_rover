...
int main(int argc, char **argv) {
  ...
  char const *hello = "/gesture baibai"; //ojigi";
  char const *voice_hello = "/say_eng { see you }"; 
  char buffer[1024] = {0};
  ...
  const float forwardingDuration = 14.0;
  const float rotatingDuration = 9.0;
  const float waitingDuration = 5.0;
  ...
  while(ros::ok()) {
	...
	send(sock , hello , strlen(hello) , 0 ); 
      ...
      if(difftime(time(0), afterIniTime) > waitingDuration && difftime(time(0), afterIniTime) < rotatingDuration){
	msg.angular.z = 0.8;
      }    
      if(difftime(time(0), afterIniTime) > rotatingDuration && difftime(time(0), afterIniTime) < forwardingDuration){
