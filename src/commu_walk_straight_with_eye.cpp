...
int main(int argc, char **argv) {
  ...
  while(ros::ok()) {
	...
	if(difference <= 1500){
	  ...
	  int const period_gest = 250;
	  if(((int)difference % period_gest >= 0) && ((int)difference % period_gest <= (period_gest-100)) && (kyoroLoop == true)){
	    kyoroLoop = false;
	    walk_cnt++;
	    ROS_INFO("walk_cnt = %d", walk_cnt);
	    if(walk_cnt % 5 == 0) {send(sock , gst_walk_eye , strlen(gst_walk_eye), 0 );ROS_INFO("blink eye!!! \n");}
	    else {send(sock , gst_walk , strlen(gst_walk) , 0 );}

	    //ROS_INFO("send\n");
	  }
	
       
	  else if((int)difference % period_gest > (period_gest-100)) kyoroLoop = true;
