...
int main(int argc, char **argv) {
  ...
  char const *hello = "/gesture hello_no_waist"; //ojigi";
  char const *voice_hello = "/say_eng { hello, nice to meet you }"; 
  char buffer[1024] = {0};
  ...
  while(ros::ok()) {
	...
	send(sock , hello , strlen(hello) , 0 );
      ...
      //cylinder movement
      const int cylPeriod = 1;
      const float cylSpeed = 0.04;
      if(difftime(time(0), afterIniTime) < forwardingDuration) {
	if((int)difftime(time(0), afterIniTime) % (2*cylPeriod) < cylPeriod ) msg.linear.z = cylSpeed;
