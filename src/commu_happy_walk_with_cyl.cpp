...
int main(int argc, char **argv) {
  ...
  char const *hello = "/gesture happy_walk"; //kyorokyoro";
  ...
  double difference;
  long after;
  double jumpSpeed = 0.1;
  double jumpSpeed2 = 0.1;
  ...
  while(ros::ok()) {
	...
	if((((int)difference % 99 >= 0) && ((int)difference % 99 <= 5) && (kyoroLoop == true)) ||( ((int)difference % 99 >= 95) && ((int)difference % 99 <= 99) && (kyoroLoop == true))){
	  kyoroLoop = false;
	...
	else if((int)difference % 99 >= 15) kyoroLoop = true;
      
	if(((int)difference % 99 >= 0) && ((int)difference % 99 < 33)){
	  //kyoroTime = time(0);
	  ROS_INFO("1 diff : %lf",difference);  
	  msg.linear.z =jumpSpeed;
	  msg.linear.x = 0.16; 
	}
	else if(((int)difference % 99 >= 33) && ((int)difference % 99 < 66)){
	  //if(difftime(time(0), kyoroTime) > 14){
	  msg.linear.z =-jumpSpeed;
	  msg.linear.x = 0.12; 
	  ROS_INFO("2 diff : %lf",difference);
	}
	else if(((int)difference % 99 >= 66) && ((int)difference % 99 < 78)){
	  msg.linear.z =jumpSpeed2;
	  msg.linear.x = 0.12; 
	  ROS_INFO("3 diff : %lf",difference);
	}
	else if(((int)difference % 99 >= 78) && ((int)difference % 99 < 99)){
	  msg.linear.z =-jumpSpeed2;
	  msg.linear.x = 0.12; 
